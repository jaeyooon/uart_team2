...
|조재원|⦁반응형 웹 템플릿을 사용했음에도 모바일 버전에서 화면 깨짐 현상이 발생함.<br>⦁카카오톡 공유하기 버튼을 한번 이상 누를 때 Kakao.init오류 발생.|⦁템플릿의 고유한 틀을 벗어나서 외부에서 반응형 탭/캐러셀등을 삽입했을 때 이러한 현상이 발생할 수 있으므로 @media작업을 통해 해결해줘야 함.<br>⦁initialize가 두번 호출 되는 경우 오류가 발생하므로 초기화 되었을 때에는 init을 호출하지 않고, 초기화 되지 않은 상황에서만 호출되도록 if문을 사용해줌.| 
