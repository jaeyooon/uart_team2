...
|김서윤|위시리스트 DB 구축 및 CRUD 설계<br>티켓 DB & 예매 DB & 예매내역 DB 수정 <br >소비자 상세정보페이지 상단부<br> 소비자 예매페이지(티켓수량선택, 결제예정 티켓수량/티켓금액/사용자정보) <br> 소비자 페이지 위시리스트 조회/추가/중복방지/삭제<br> 관리자 페이지 (매출 차트 전체/실시간 서버시간/가입자수/리뷰별 추천전시 롤링배너) |
...
|김서윤|⦁ localhost에서 작동되는 group by가 포함된 쿼리가 NCP서버에서는 1055 에러로 작동되지 않아서 차트가 나오지 않음.<br><br> ⦁ 하나의 티켓에서 대상이 성인,청소년,어린이로 나누어지고 가격도 3개가 나와야 하는데 db에 어떻게 나타내야 이클립스에서 구동하기 쉬울 지 모르겠음. |⦁ 방법1. group by 사용 시 select의 칼럼 중 집계함수에 쓰이는 칼럼을 제외한 모든 칼럼을 기입해야 함. <br> ⦁ 방법2. $ vi /etc/my.cnf 에서 sql_mode에서 ONLY_FULL_GROUP_BY 설정을 제거해야 함. <br><br> ⦁ 대상 칼럼도 3개, 가격 칼럼도 3개 만들면 됨.&nbsp; 그래서 insert 한번에 해당 티켓의 대상 3개, 가격 3개가 다 들어가야함.&nbsp; 대상 칼럼 1개, 가격 칼럼 1개 만드는 경우 대상 3개, 가격 3개 넣으려면 insert 3번 해야함.&nbsp; 이러면 이클립스에서 이용하기 어려움(가능하기는 함). |
