...
|김서윤|⦁ localhost에서 작동되는 group by가 포함된 쿼리가 NCP서버에서는 1055 에러로 작동되지 않아서 차트가 나오지 않음.<br> ⦁ 하나의 티켓에서 대상이 성인,청소년,어린이로 나누어지고 가격도 3개가 나와야 하는데 db에 어떻게 나타내야 이클립스에서 구동하기 쉬울지 모르겠음. |⦁ 방법1. group by 사용 시 select의 칼럼 중 집계함수에 쓰이는 칼럼을 제외한 모든 칼럼을 기입해야 함. <br> ⦁ 방법2. $ vi /etc/my.cnf 에서 sql_mode에서 ONLY_FULL_GROUP_BY 설정을 제거. <br>  <br> ⦁ 대상 칼럼도 3개, 가격 칼럼도 3개 만들면 됨. 대상 칼럼 1개, 가격 칼럼 1개 만들고 서로 다른 대상, 가격을 넣으면 이클립스에서 이용하기 어려움(가능하기는 함). |
